...
class PostgresTradeStore:
    ...
    def init_schema(self) -> None:
        statements = [
            ...
            CREATE TABLE IF NOT EXISTS trades (
                coin TEXT NOT NULL, time_ms BIGINT NOT NULL, tid BIGINT NOT NULL,
                side TEXT NOT NULL CHECK (side IN ('B', 'A')),
                px DOUBLE PRECISION NOT NULL, sz DOUBLE PRECISION NOT NULL,
                notional_usdc DOUBLE PRECISION NOT NULL,
                signed_notional_usdc DOUBLE PRECISION NOT NULL,
                trade_hash TEXT, inserted_at_ms BIGINT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS coverage_gaps (
                id BIGSERIAL PRIMARY KEY, coin TEXT NOT NULL, start_ms BIGINT NOT NULL,
                end_ms BIGINT NOT NULL, status TEXT NOT NULL CHECK (status IN ('HEALED','UNRESOLVED')),
                reason TEXT NOT NULL, recovery_earliest_ms BIGINT, recovery_latest_ms BIGINT,
                recovered_trade_count INTEGER NOT NULL DEFAULT 0, created_at_ms BIGINT NOT NULL,
            ...
            CREATE TABLE IF NOT EXISTS aggregate_buckets (
                ...
                trade_count BIGINT NOT NULL, buy_notional_usdc DOUBLE PRECISION NOT NULL,
                sell_notional_usdc DOUBLE PRECISION NOT NULL, net_delta_usdc DOUBLE PRECISION NOT NULL,
                base_delta_hype DOUBLE PRECISION NOT NULL, total_notional_usdc DOUBLE PRECISION NOT NULL,
                delta_ratio DOUBLE PRECISION, first_trade_time_ms BIGINT, last_trade_time_ms BIGINT,
                complete BOOLEAN NOT NULL, quality TEXT NOT NULL, unresolved_gap_count INTEGER NOT NULL,
    ...
    def insert_many(self, records: Iterable[TradeRecord]) -> tuple[int, int]:
        rows = [(r.coin,r.time_ms,r.tid,r.side,r.px,r.sz,r.notional_usdc,r.signed_notional_usdc,r.trade_hash,int(time.time()*1000)) for r in records]
        if not rows:
            return 0, 0
        sql = """
            INSERT INTO trades (coin,time_ms,tid,side,px,sz,notional_usdc,signed_notional_usdc,trade_hash,inserted_at_ms)
        ...
        with self._lock, self.conn.cursor() as cur:
            cur.execute(
                ...
                SELECT COUNT(*) AS trade_count,
                    ...
                    MIN(time_ms) AS first_trade_time_ms, MAX(time_ms) AS last_trade_time_ms
        ...
        with self._lock, self.conn.cursor() as cur:
            cur.execute(
                ...
                INSERT INTO aggregate_buckets(
                    coin,granularity,start_ms,end_ms,trade_count,buy_notional_usdc,sell_notional_usdc,
                    net_delta_usdc,base_delta_hype,total_notional_usdc,delta_ratio,first_trade_time_ms,
                    last_trade_time_ms,complete,quality,unresolved_gap_count,created_at_ms,updated_at_ms
                ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                ON CONFLICT (coin,granularity,start_ms) DO UPDATE SET
                    end_ms=EXCLUDED.end_ms, trade_count=EXCLUDED.trade_count,
                    buy_notional_usdc=EXCLUDED.buy_notional_usdc, sell_notional_usdc=EXCLUDED.sell_notional_usdc,
                    net_delta_usdc=EXCLUDED.net_delta_usdc, base_delta_hype=EXCLUDED.base_delta_hype,
                    total_notional_usdc=EXCLUDED.total_notional_usdc, delta_ratio=EXCLUDED.delta_ratio,
                    first_trade_time_ms=EXCLUDED.first_trade_time_ms, last_trade_time_ms=EXCLUDED.last_trade_time_ms,
                    complete=EXCLUDED.complete, quality=EXCLUDED.quality,
                    unresolved_gap_count=EXCLUDED.unresolved_gap_count, updated_at_ms=EXCLUDED.updated_at_ms
                """,
                (coin,granularity,start_ms,end_ms,int(stats["trade_count"]),float(stats["buy_notional_usdc"]),
                 float(stats["sell_notional_usdc"]),float(stats["net_delta_usdc"]),float(stats["base_delta_hype"]),
                 float(stats["total_notional_usdc"]),stats["delta_ratio"],stats["first_trade_time_ms"],
                 stats["last_trade_time_ms"],bool(complete),quality,unresolved_gap_count,now_ms,now_ms),
            )
            self.conn.commit()
    ...
    def set_meta(self, key: str, value: str | int) -> None:
        with self._lock, self.conn.cursor() as cur:
            cur.execute("INSERT INTO collector_meta(key,value) VALUES(%s,%s) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value", (key,str(value)))
            self.conn.commit()
    ...
    def add_gap(self, coin: str, start_ms: int, end_ms: int, *, status: str, reason: str, recovery_earliest_ms: int | None = None, recovery_latest_ms: int | None = None, recovered_trade_count: int = 0) -> None:
        ...
        with self._lock, self.conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO coverage_gaps(coin,start_ms,end_ms,status,reason,recovery_earliest_ms,recovery_latest_ms,recovered_trade_count,created_at_ms)
    ...
    def gaps_overlapping(self, coin: str, start_ms: int, end_ms: int, *, status: str = "UNRESOLVED") -> list[dict]:
        with self._lock, self.conn.cursor() as cur:
            cur.execute("SELECT * FROM coverage_gaps WHERE coin=%s AND status=%s AND end_ms>%s AND start_ms<%s ORDER BY start_ms", (coin,status,start_ms,end_ms))

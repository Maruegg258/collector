...
class HypeSpotCollector:
    def __init__(
        ...
        *,
        ...
        self._pending_gap_start_ms = (
            None if lease is not None else store.get_meta_int("coverage_heartbeat_ms")
        )
        ...
        if self.lease is not None:
            if force or now_ms - self._last_heartbeat_persist_ms >= 5_000:
                self.lease.update(
                    ...
                    heartbeat_ms=now_ms,
                    last_message_at_ms=self.state.last_message_at_ms,
                )
    ...
    def _mark_unexpected_disconnect(self) -> int | None:
        if self.lease is not None:
            ...
            gap_start_ms = int(
                own.get("own_heartbeat_ms")
                or self.state.last_message_at_ms
    ...
    async def _recover_gap(self, gap_start_ms: int, gap_end_ms: int) -> None:
        ...
        try:
            ...
            self.store.add_gap(
                ...
                gap_end_ms,
                status=status,
                reason="recentTrades_overlap_proof" if healed else "recentTrades_insufficient_overlap",
            ...
            try:
                ...
                if self.state.connected:
                    if self._stop.is_set():
                        ...
                        if gap_start_ms is not None:
                            logger.warning(
                                "continuity_gap_opened start_ms=%s basis=last_healthy_lease_or_message",
                                gap_start_ms,
                            )
